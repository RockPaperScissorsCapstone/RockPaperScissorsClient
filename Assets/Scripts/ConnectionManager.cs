...
namespace ServerManager
{
    public class ConnectionManager
    {
        ...
        public int StartClient()
        {
            ...
            // //Production (Krishna's Server)
            // ipHostInfo = Dns.GetHostEntry("ec2-18-220-56-132.us-east-2.compute.amazonaws.com");
        ...
        public string GetUsernameFromPlayerID(string username)
        {
            string[] response = new string[3];
            Console.WriteLine("Socket connected to {0}", sender.RemoteEndPoint.ToString());
            //Data buffer for incoming data.
            byte[] msgFunction = EncodeToBytes("GetUsernameFromPlayerID");
            response[0] = Messenger(msgFunction);
        ...
        public string GetScore(string param)
        {
            string[] results = new string[3];

            byte[] msgFunction = EncodeToBytes("GetScore");
            results[0] = Messenger(msgFunction);

            byte[] msgUserID = EncodeToBytes(param);
            results[1] = Messenger(msgUserID);
